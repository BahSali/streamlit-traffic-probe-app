...
def load_segments_metadata(path: str = METADATA_PATH) -> pd.DataFrame:
    ...
    # Build the segment label used in the left filter.
    df["segment_name"] = (
    ...
    # Normalize bus lines into a list of strings.
...
def load_brussels_map(path: str = MAP_PATH) -> gpd.GeoDataFrame:
    ...
    gdf["bus_line_list"] = gdf["bus_lines"].apply(split_bus_lines)
...
def prepare_three_map_geojson(
    ...
    def row_matches_selected(row) -> bool:
        segment_match = False
        bus_match = False

        if selected_segment_names:
            segment_match = row["segment_name_display"] in selected_segment_names

        if selected_bus_ids:
            bus_match = any(bus_id in selected_bus_ids for bus_id in row["bus_line_list"])

        return segment_match or bus_match

    # Map 1 and Map 2: fully colorized when enabled.
    if colorized:
    ...
    # Map 3: colorize only selected segments.
    google_speed_values = []
    for i, row in gdf.iterrows():
        if colorized and row_matches_selected(row):
